...
- Decision: Gate the bypass at the privilege manager, but only enable the context from plan replayer's internal helper SQL wrappers.
  Rationale: this keeps normal SQL behavior unchanged while still covering both `EXPLAIN` and `SHOW CREATE` code paths, including view expansion. The bypass now acts only as a fallback after the built-in object privilege check fails, and auth-plugin static privilege checks still apply.
  Date/Author: 2026-04-01 / Codex
...
On the plan replayer side, let manual, list-based, and file-input dump tasks share the same parsed-statement path. Wrap internal `EXPLAIN` and `SHOW CREATE` execution in helpers that temporarily enable the session context only for fallback retries after privilege errors. Only allow the bypass helper for non-`ANALYZE` tasks whose inner statements are all real `SELECT` statements; set operations are allowed only when every leaf operand is a real `SELECT`, and `TABLE` / `VALUES` stay outside the allowlist even when nested inside `UNION` / `INTERSECT` / `EXCEPT`, derived tables, subqueries, or CTEs. While the bypass context is active for `EXPLAIN`, temporarily disable non-evaluated scalar subquery explain output so the plan output does not embed real scalar values.
